...
let legendNameData = computed(() => `${props.selectBtmTab.label}点数`);
...
const addQuBar = async () => {
  ...
  rectBar4Ref.value?.addBar('qu', res, '', props.selectBtmTab.code);
};
let fieldTrans = {
...
const addOutStation = async (module: number, jdcode: string) => {
  console.log(module, jdcode, '11111111111111');
  await __g.marker.deleteByGroupId('jdStation');
  const { data: res } = await Api.getStreetBar({ areaCode: '440303', streetCode: '440303002000' });
  let pointArr = [];
...
onMounted(async () => {
  bus.on('addBar', async (e) => {
    try {
      ...
      rectBar4Ref.value.addBar(e.type, res, e.quCode, props.selectBtmTab.code);
    } catch (err) {
